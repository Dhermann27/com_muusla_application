...
class muusla_applicationModelapplication extends JModel
{
   ...
   function getWorkshops() {
      $db =& JFactory::getDBO();
      $query = "SELECT w.timeslotid, CONCAT(IF(w.su,'S',''),IF(w.m,'M',''),IF(w.t,'Tu',''),IF(w.w,'W',''),IF(w.th,'Th',''),IF(w.f,'F',''),IF(w.sa,'S','')) days, w.id, w.name, w.subname, jc.introtext FROM muusa_workshop w LEFT JOIN jml_content jc ON REPLACE(jc.title, '&', '') LIKE CONCAT('%', REPLACE(w.name, '&', ''), '%') AND jc.state=1 ORDER BY w.name";
      $db->setQuery($query);
      return $db->loadAssocList("id");
   }
   ...
   function getRegisteredCampersByFamily($where) {
      $db =& JFactory::getDBO();
      $user =& JFactory::getUser();
      $query = "SELECT id, firstname, lastname, yearattendingid, IF(grade<13,0,1) shops, programname FROM muusa_thisyear_camper WHERE $where ORDER BY STR_TO_DATE(c.birthdate, '%m/%d/%Y')";
      $db->setQuery($query);
      return $db->loadObjectList();
   }
